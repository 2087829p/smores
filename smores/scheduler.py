...
class RankingFilter(st.Filter):
    ...
    def __train_and_rank__(self, now):
        influential_users = [(self._classifier.predict(np.array(self._influential_users[u].values())), u) for u in
        ...
        samples = random.sample(out[1], max(len(out[1]), max_number_of_tweets_per_cycle))
    ...
    def process(self, data):
        ...
        for t in data:
            user = self._influential_users.get(t["id"],
...
class Scheduler:
    ...
    def __setup_streamer__(self, trend_data):
        ...
        store = self._store if not self._predefined_store else self._storages['stream'].write
    ...
    def __prepare_twitter__(self, tasks, login, proxies):
        ...
        users = [l for l in ug]
        store = self._store if not self._predefined_store else self._storages['model'].write
        ...
        # t = self._handlers[i % len(self._handlers)]
        ...
        if self._use == TWITTER_HYBRID_MODEL and self._target_queue['users'] and task in [TASK_FETCH_USER,
            ...
            if task == TASK_BULK_RETRIEVE:
                predicted = [self.__get_top_n_accounts__(TWITTER_BULK_LIST_SIZE) for _ in
                             range(int(TWITTER_MAX_NUM_OF_BULK_LISTS_PER_REQUEST_CYCLE * PARTITION_FACTOR))]
